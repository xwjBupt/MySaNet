...
#测试
path_img = '/media/xwj/Data/DataSet/shanghai_tech/original/part_A_final/train_data/images/'
gt_path = '/media/xwj/Data/DataSet/shanghai_tech/original/part_A_final/train_data/ground_truth/'

train_path_img = '/media/xwj/Data/DataSet/shanghai_tech/raw/part_A_final/train_A/images/'
train_path_den = '/media/xwj/Data/DataSet/shanghai_tech/raw/part_A_final/train_A/den/'
val_path_img = '/media/xwj/Data/DataSet/shanghai_tech/raw//part_A_final/val_A/images/'
val_path_den =  '/media/xwj/Data/DataSet/shanghai_tech/raw/part_A_final/val_A/den/'
...
for idx in range(1, num_images+1):
    ...
    h, w = im.shape
    wn2, hn2 = int(w / 8) * 8, int(h / 8) * 8
    annPoints =  image_info[0][0][0][0][0] - 1
    im = cv2.resize(im, (wn2, hn2), interpolation=cv2.INTER_LANCZOS4)
    ...
    if idx < num_val:
        cv2.imwrite(''.join([val_path_img, str(i), '.jpg']), im)
        with open(''.join([val_path_den, str(i), '.csv']), 'w') as fout:
            writer = csv.writer(fout)
            writer.writerows(im_density)
    else:
        cv2.imwrite(''.join([train_path_img, str(i), '.jpg']), im)
        with open(''.join([train_path_den, str(i), '.csv']), 'w') as fout:
            writer = csv.writer(fout)
            writer.writerows(im_density)
